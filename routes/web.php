...
//Route::get('/75d5424a-2655-498f-b283-1730627a9f8d', function () {
//    $ProductsImages = \App\Models\Product::select('id', 'thumbnail', 'photo')->paginate(80)->toArray();
//
////    $ProductsImagesCollect = collect($ProductsImages)->map(function ($item) {
////        return ['id' => $item['id'], 'path' => public_path('/assets/images/thumbnails/' . $item['thumbnail'])];
////    });
//
//    $ProductsImagesCollect = collect($ProductsImages['data'])->map(function ($item) {
//        return ['path_1' => public_path('/assets/images/thumbnails/' . $item['thumbnail']), 'path_2' => public_path('/assets/images/products/' . $item['photo'])];
//    });
//dd($ProductsImagesCollect);
//    $tmpFile = tempnam('/tmp', '');
//
//    $zip = new ZipArchive;
//    $zip -> open($tmpFile, ZipArchive::CREATE);
//
//    foreach ($ProductsImagesCollect as $file) {
//        if (file_exists($file)){
//            // download file
...
//    unlink($tmpFile);
//
////    foreach ($ProductsImagesCollect as $key => $file) {
////        if (file_exists($file['path'])){
////            $fl[$key]['id'] = $file['id'];
////            $fl[$key]['path'] = $file['path'];
////        }
////    }
////    dd($fl);
//});
...
Route::get('/testtesttest', function () {
  return dd(\App\Classes\Helper::GetFAQTranslateContentBetweenTag('{ar}تجربه نصيه{/ar} {en}test text{/en}'));
});

// Route::get('/images2', function (){

//   set_time_limit(0);
//     // $products = \App\Models\Product::where([
//     //  ['status',1],['thumbnail',''],['photo','']
//     //  ])->get();
...
//     //  }
//     //  dd($products);
//   $products = \App\Models\Product::where('status',1)->paginate(9000);

//   list($noImageThum, $withimageThum) = $products->partition(function ($i) {
//     if( !file_exists(public_path('assets/images/thumbnails/'.$i->thumbnail)) ){
//         return true;
//     }
//   });

//   list($noImagePhoto, $withimagePhoto) = $noImageThum->partition(function ($i) {
//     if( !file_exists(public_path('assets/images/products/'.$i->photo))  ){
//         return true;
//     }
//   });

// //      foreach($withimagePhoto as $pro){
// //      $pro->update([
// //       'status' => 1
// //      ]);
// //   }

//   dd($withimageThum, $withimagePhoto, $noImagePhoto);

// });

Route::get('/admin/orders-check/{id}', function (Request $request) {
  ...
  foreach ($orders as $order) {
    ...
    if (in_array((int)$request->id, array_keys($cart->items))) {
      $order_details = [
      ...
      array_push($order_ids, $order_details);
    }
  }

  if (count($order_ids) == 0) {
    return redirect()->route('admin.dashboard')->with('unsuccess', __('Product Has No Orders.'));
  }

  return view('admin.product.print', ['orders' => $order_ids, 'product' => $product]);

  // return response()->json($order_ids);

  // dd($order_ids);
  // header('Content-Type: application/csv; charset=UTF-8');
  // header('Content-Disposition: attachment; filename="product_order.csv";');

  // // open the "output" stream
  // // see http://www.php.net/manual/en/wrappers.php.php#refsect2-wrappers.php-unknown-unknown-unknown-descriptioq
  // $f = fopen('php://output', 'w');

  // foreach ($order_ids as $line) {
  //     fputcsv($f, $line);
  // }
  // fclose($f);

})->middleware('auth:admin')->name('product.orders');
/*
Route::get('/admin/orders-update/qty', function(Request $request){
    ...
    // 50 450
    // 50 500
    $orders = \App\Models\PreOrder::take(50)->skip(0)->get();

    foreach($orders as $order){
        $cart = unserialize(bzdecompress(utf8_decode($order->cart)));
        
        $cart = new \App\Models\Cart($cart);
        $qty = collect($cart->items)->sum('qty');
        $cart->totalQty = $qty;
        $order->totalQty = $qty;
...
Route::prefix('admin')->group(function () {
  ...
  // User Notification
  Route::get('/action_filter_brand', 'Admin\DashboardController@MoveBrand')->name('action_filter_brand');
  Route::get('/action_filter_delete', 'Admin\DashboardController@ActionFilterDelete')->name('action_filter_delete');
  Route::get('/action_filter', 'Admin\DashboardController@ActionFilter')->name('action_filter');
  ...
  Route::group(['middleware' => 'permissions:orders'], function () {
    ...
    Route::get('/orders/on-delivery', 'Admin\OrderController@onDelivery')->name('admin-order-onDelivery');
    ...
    Route::get('/pre-orders/datatables/{slug}', 'Admin\PreOrderController@datatables')->name('admin-preOrder-datatables'); //JSON REQUEST
    Route::get('/pre-orders', 'Admin\PreOrderController@index')->name('admin-preOrder-index');
    Route::get('/pre-order/{id}/show', 'Admin\PreOrderController@show')->name('admin-preOrder-show');
    Route::get('/pre-order/{id}/move', 'Admin\PreOrderController@move')->name('admin-preOrder-move');
  ...
  Route::group(['middleware' => 'permissions:products'], function () {
    ...
    Route::get('/products/datatables995/{id}', 'Admin\ProductController@datatables9995')->name('admin-prod-datatables9995');
    Route::get('/products/datatables/{id}', 'Admin\ProductController@datatables95')->name('admin-prod-datatables95');
    Route::get('/products/datatables955', 'Admin\ProductController@datatables955')->name('admin-prod-datatables955');
    ...
    Route::get('/brands', 'Admin\ProductController@brands')->name('admin-brand-index');
    Route::post('/add/brands', 'Admin\ProductController@BrandAdd')->name('action_add_brand');
    ...
    Route::get('/brands/products/delete/{id}', 'Admin\ProductController@BrandProductdestroy')->name('admin-prod-brand-delete');
    Route::get('/brands/delete/{id}', 'Admin\ProductController@destroybrand')->name('admin-brand-delete');
    Route::get('/brands/edit/{id}', 'Admin\ProductController@editbrand')->name('admin-brand-edit');
    Route::post('/brands/update/{id}', 'Admin\ProductController@updatebrand')->name('admin-brand-update');
    // DELETE SECTION ENDS
  ...
  Route::group(['middleware' => 'permissions:categories'], function () {
    ...
    Route::get('/branches', 'Admin\BranchController@index')->name('admin-branches-index');
    Route::get('/branches/datatables', 'Admin\BranchController@datatables')->name('admin-branches-datatables'); //JSON REQUEST
    Route::get('/branches/create', 'Admin\BranchController@create')->name('admin-branch-create');
    Route::post('/branches/create', 'Admin\BranchController@store')->name('admin-branch-store');

    Route::get('/branches/edit/{id}', 'Admin\BranchController@edit')->name('admin-branches-edit');
    Route::post('/branches/edit/{id}', 'Admin\BranchController@update')->name('admin-branches-update');
    Route::get('/branches/delete/{id}', 'Admin\BranchController@destroy')->name('admin-branches-delete');
    ...
    Route::get('/Category/products/{catid}/', 'Admin\ProductController@CategoryProducts')->name('admin-category-products');
    Route::get('/Brands/products/{catid}/', 'Admin\ProductController@BrandsProducts')->name('admin-brands-products');
  ...
  Route::group(['middleware' => 'permissions:set_coupons'], function () {
    ...
    Route::get('/coupon/{id}/orders', 'Admin\CouponController@orders')->name('admin-coupon-orders');
  });
  ...
  Route::group(['middleware' => 'permissions:messages'], function () {
    ...
    Route::get('/admin/prescription', 'Admin\MessageController@prescription')->name('admin-message-prescription');
  });
  ...
  Route::group(['middleware' => 'permissions:general_settings'], function () {
    ...
    Route::get('/general-settings/sms-verify/{status}', 'Admin\GeneralSettingController@isSmsVerify')->name('admin-gs-is-sms-verify');
  ...
  Route::group(['middleware' => 'permissions:home_page_settings'], function () {
    ...
    Route::get('top/three/banner/', 'Admin\BannerController@top')->name('admin-top-bottom');
    Route::get('home/category/banner/', 'Admin\BannerController@homeCat')->name('admin-home-categories');
    ...
    Route::get('top/three/banner/create', 'Admin\BannerController@topcreate')->name('admin-sb-create-top');
    Route::get('home/category/banner/create', 'Admin\BannerController@homecatcreate')->name('admin-sb-create-homecat');
  ...
  Route::group(['middleware' => 'permissions:super'], function () {
    ...
    Route::get('/cache/clear', function () {
      ...
      // Artisan::call('migrate',['--path' =>'database/migrations/new']);
...
Route::prefix('user')->group(function () {
  ...
  // User Register
  Route::get('/register/verify/sms', 'User\RegisterController@smsCode')->name('user-verify-sms');
  Route::post('/register/sms/check', 'User\RegisterController@smsCheck')->name('user-sms-check');
  ...
  // User Logout
  Route::get('/delete-account', 'User\LoginController@deleteAccount')->name('user-delete-account');
...
Route::get('/sitemap.xml', function () {
  return response()->view('front.sitemaps')->header('Content-Type', 'application/xml');
});
...
Route::group(['middleware' => 'maintenance'], function () {
  ...
  Route::get('/quick-deal', 'Front\FrontendController@quickDeal')->name('front.quickDeal');
  Route::get('/page/quick-deal', 'Front\FrontendController@quickDeal2')->name('front.page.quickDeal');
  ...
  Route::get('/brand/{id}/products', 'Front\FrontendController@brandProducts')->name('front.brandProducts');
  ...
  // CONTACT SECTION front.branches
  Route::get('/contact', 'Front\FrontendController@contact')->name('front.contact');
  Route::get('/branches', 'Front\FrontendController@branches')->name('front.branches');
  Route::get('/pharmacy', 'Front\FrontendController@pharmacy')->name('front.pharmacy');
  Route::get('/offers/{id}', 'Front\FrontendController@offers');
  Route::get('/receipt', 'Front\FrontendController@receipt')->name('front.receipt');
  Route::post('/contact', 'Front\FrontendController@contactemail')->name('front.contact.submit');
  Route::post('/pharmacy', 'Front\FrontendController@pharmcyemail')->name('front.pharmcy.submit');
  Route::post('/preception', 'Front\FrontendController@preception')->name('front.preception.submit');
  ...
  Route::get('/checkout/paymenterror', 'Front\CheckoutController@loadPaymentErrors');
  ...
  Route::get('/checkout/payment/return2', 'Front\PaymentController@payreturn2')->name('payment.return2');
  ...
  Route::delete('/bulck-delete-multiple/{table}', function (Request $request, $table) {
    try {
      $ids = $request->ids;
      DB::table($table)->whereIn('id', explode(",", $ids))->delete();
      return response()->json(['success' => ucwords($table) . " Deleted successfully."]);
    } catch (\Throwable $throwable) {
      return response()->json(['error' => "An unknown error has occurred."]);
    }
  })->name('BulkDeleteMultiple');

  Route::get('/export/export-products-data', function () {
    $fileName = 'products.csv';
    $products = \App\Models\Product::select('id', 'name_en', 'name_ar', 'price', 'previous_price', 'details_en', 'details_ar', 'photo')->get();
    ...
    $columns = ['id', 'Name en', 'Name ar', 'Price', 'Previous Price', 'Description en', 'Description ar', 'image'];
    ...
    $callback = function () use ($products, $columns) {
      ...
      foreach ($products as $task) {
        $row['id']  = $task->id;
        $row['name_en']    = $task->name_en;
        $row['name_ar']    = $task->name_ar;
        $row['price']  = $task->price;
        $row['previous_price']  = $task->previous_price;
        $row['details_en']  = $task->details_en;
        $row['details_ar']  = $task->details_ar;
        $row['photo']  = $task->photo;

        fputcsv($file, [$row['id'], $row['name_en'], $row['name_ar'], $row['price'], $row['previous_price'], $row['details_en'], $row['details_ar'], $row['photo']]);
      }
